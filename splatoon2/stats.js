...
angular.module('splatApp').stats = function ($scope) {
  $scope.stats = {
    ...
    'Main Power Up 1': new Stat("Main Power Up", function(loadout) {
      ...
      //     this.label = "{value}%".format({value: $scope.toFixedTrimmed(result,3)});
      //     label_set = true;
      //   }
      ...
      //     this.percentage = ((result/min_param - 1) * 100).toFixed(1);
      //     result = result / 60.0;
      //     this.label = "{value}s".format({value: $scope.toFixedTrimmed(result,2)});
      //     label_set = true;
      //   }
      ...
      //     this.value = $scope.toFixedTrimmed((result/max_param) * 100,2);
      //     this.percentage = ((result/min_param - 1) * 100).toFixed(1);
      //     this.label = "{value}".format({value: $scope.toFixedTrimmed(result,2)});
      //   }
      ...
      this.value = 0;
      this.name = "主武器增强 (未用)";
      this.label = "不可用".format({value: this.value});
      this.desc = null;
      return this.value;
    }, 100),
  ...
  var inkResistanceEffects = [
    {"name":"免疫时间","key":"Invuln Frames"},
    {"name":"每秒伤害","key":"Dmg Per Frame"},
    {"name":"伤害极限", "key":"Dmg Limit"},
    {"name":"跳跃高度", "key":"Jump"}      
  ];
